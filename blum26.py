...
bot_token = "7511944215:AAF4CeMJpCNBuDdSVwHCtC8Pnjbq59b0pSo"
chat_id = "603122129"
...
def check_tribe(access_token,retries=3,delay=2):
    url = f"https://game-domain.blum.codes/api/v1/tribe/my"
    headers = get_headers(access_token)
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"{RED}[ Tribe ] : Error: Gagal mendapatkan tribe", flush=True)
                return None
        except (requests.RequestException, ValueError) as e:
            if attempt < retries - 1:
                print(f"{YELLOW}[ Tribe ] : Error Retrying... ({attempt + 1}/{retries})", end="\r", flush=True)
                time.sleep(delay)
            else:
                return None     
...
def join_tribe(access_token, tribe_id, retries=3, delay=2):
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                print(f"{RED}[ Tribe ] : Error Retrying... ({attempt + 1}/{retries})", end="\r", flush=True)
                time.sleep(delay)
            else:
                print(f"{RED}[ Tribe ] : Failed to Join Tribe", flush=True)
